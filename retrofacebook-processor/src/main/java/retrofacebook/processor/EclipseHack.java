 ...
 * the class with the {@code @RetroFacebook} annotation, and we do just enough parsing of that code to
 ...
 * {@code @RetroFacebook} constructor calls like {@code new RetroFacebook_Foo(...)} suddenly start being
...
class EclipseHack {
   ...
   * has no effect.
   */
  void reorderProperties(List<RetroFacebookProcessor.Property> properties) {
    ...
    while (index < properties.size()) {
      ...
      List<RetroFacebookProcessor.Property> subList = properties.subList(index, nextIndex);
