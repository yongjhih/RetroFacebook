 ...
 * @see retrofacebook.RetroFacebook
 */
@AutoService(Processor.class)
public class RetroFacebookProcessor extends AbstractProcessor {
  public RetroFacebookProcessor() {
  }

  @Override
  public Set<String> getSupportedAnnotationTypes() {
    return ImmutableSet.of(RetroFacebook.class.getName());
  }
  ...
  /**
   * Qualified names of {@code @RetroFacebook} classes that we attempted to process but had to abandon
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    ...
    if (roundEnv.processingOver()) {
      ...
      // any new instances of @RetroFacebook; and we can't have any new types that are the reason a type
      // was in deferredTypes.
      for (TypeElement type : deferredTypes) {
        errorReporter.reportError("Did not generate @RetroFacebook class for " + type.getQualifiedName()
            + " because it references undefined types", type);
      }
      return false;
    }
    Collection<? extends Element> annotatedElements =
        roundEnv.getElementsAnnotatedWith(RetroFacebook.class);
    ...
    for (TypeElement type : types) {
      try {
        ...
        // again and adding it back to the list). We save the name of the @RetroFacebook type rather
        ...
        errorReporter.reportError("@RetroFacebook processor threw an exception: " + trace, type);
      }
    }
  ...
  public static class Property {
    ...
    /**
     * Returns the name of the getter method for this property as defined by the {@code @RetroFacebook}
  ...
  private void processType(TypeElement type) {
    RetroFacebook autoValue = type.getAnnotation(RetroFacebook.class);
    if (autoValue == null) {
      // This shouldn't happen unless the compilation environment is buggy,
      // but it has happened in the past and can crash the compiler.
      errorReporter.abortWithError("annotation processor for @RetroFacebook was invoked with a type"
          + " that does not have that annotation; this is probably a compiler bug", type);
    }
    if (type.getKind() != ElementKind.CLASS) {
      errorReporter.abortWithError(
          "@" + RetroFacebook.class.getName() + " only applies to classes", type);
    }
    if (ancestorIsRetroFacebook(type)) {
      errorReporter.abortWithError("One @RetroFacebook class may not extend another", type);
    }
    if (implementsAnnotation(type)) {
      errorReporter.abortWithError("@RetroFacebook may not be used to implement an annotation"
          + " interface; try using @AutoAnnotation instead", type);
    }
    RetroFacebookTemplateVars vars = new RetroFacebookTemplateVars();
    ...
    for (Map.Entry<ExecutableElement, String> entry : methodToIdentifier.entrySet()) {
      if (!seen.add(entry.getValue())) {
        errorReporter.reportError(
            "More than one @RetroFacebook property called " + entry.getValue(), entry.getKey());
      }
    }
  }
  ...
  private static void determineObjectMethodsToGenerate(
      List<ExecutableElement> methods, RetroFacebookTemplateVars vars) {
  ...
  // The @RetroFacebook type, with a ? for every type.
  // If we have @RetroFacebook abstract class Foo<T extends Something> then this method will return
  // just <?>.
