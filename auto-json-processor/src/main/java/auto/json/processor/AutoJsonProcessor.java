 ...
 * @see auto.json.AutoJson
 */
@AutoService(Processor.class)
public class AutoJsonProcessor extends AbstractProcessor {
  public AutoJsonProcessor() {
  }

  @Override
  public Set<String> getSupportedAnnotationTypes() {
    return ImmutableSet.of(AutoJson.class.getName());
  }
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    ...
    if (roundEnv.processingOver()) {
      ...
      // any new instances of @AutoJson; and we can't have any new types that are the reason a type
      // was in deferredTypes.
      for (TypeElement type : deferredTypes) {
        errorReporter.reportError("Did not generate @AutoJson class for " + type.getQualifiedName()
    ...
    for (TypeElement type : types) {
      try {
        ...
        // again and adding it back to the list). We save the name of the @AutoJson type rather
        ...
        errorReporter.reportError("@AutoJson processor threw an exception: " + trace, type);
      }
    }
  ...
  private String generatedSubclassName(TypeElement type) {
    return generatedClassName(type, "AutoJson_");
  }

  /**
   * A property of an {@code @AutoJson} class, defined by one of its abstract methods.
  ...
  private void processType(TypeElement type) {
    AutoJson autoValue = type.getAnnotation(AutoJson.class);
    if (autoValue == null) {
      // This shouldn't happen unless the compilation environment is buggy,
      // but it has happened in the past and can crash the compiler.
      errorReporter.abortWithError("annotation processor for @AutoJson was invoked with a type"
          + " that does not have that annotation; this is probably a compiler bug", type);
    }
    if (type.getKind() != ElementKind.CLASS) {
      errorReporter.abortWithError(
          "@" + AutoJson.class.getName() + " only applies to classes", type);
    }
    if (ancestorIsAutoJson(type)) {
      errorReporter.abortWithError("One @AutoJson class may not extend another", type);
    }
    if (implementsAnnotation(type)) {
      errorReporter.abortWithError("@AutoJson may not be used to implement an annotation"
          + " interface; try using @AutoAnnotation instead", type);
    }
    AutoJsonTemplateVars vars = new AutoJsonTemplateVars();
    ...
    for (Map.Entry<ExecutableElement, String> entry : methodToIdentifier.entrySet()) {
      if (!seen.add(entry.getValue())) {
        errorReporter.reportError(
            "More than one @AutoJson property called " + entry.getValue(), entry.getKey());
      }
    }
  }
  ...
  private static void determineObjectMethodsToGenerate(
      List<ExecutableElement> methods, AutoJsonTemplateVars vars) {
      ...
      if (method.getModifiers().contains(Modifier.ABSTRACT)
        ...
        if (method.getParameters().isEmpty() && method.getReturnType().getKind() != TypeKind.VOID) {
          if (isReferenceArrayType(method.getReturnType())) {
            errorReporter.reportError("An @AutoJson class cannot define an array-valued property"
          ...
          // that overrides it. This shows up in AutoJsonTest.LukesBase for example.
          errorReporter.reportWarning("@AutoJson classes cannot have abstract methods other than"
  ...
  private boolean ancestorIsAutoJson(TypeElement type) {
    while (true) {
      ...
      if (parentElement.getAnnotation(AutoJson.class) != null) {
        return true;
      }
      type = parentElement;
    }
  ...
  // If we have @AutoJson abstract class Foo<T extends Something> then this method will return
  // just <?>.
