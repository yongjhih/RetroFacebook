...
class BuilderSpec {
  ...
  /**
   * Determines if the {@code @AutoJson} class for this instance has a correct nested
   * {@code @AutoJson.Builder} class or interface and return a representation of it in an
   * {@code Optional} if so.
   */
  Optional<Builder> getBuilder() {
    ...
    for (TypeElement containedClass : ElementFilter.typesIn(autoValueClass.getEnclosedElements())) {
      if (MoreElements.isAnnotationPresent(containedClass, AutoJson.Builder.class)) {
        if (!CLASS_OR_INTERFACE.contains(containedClass.getKind())) {
          errorReporter.reportError(
              "@AutoJson.Builder can only apply to a class or an interface", containedClass);
    ...
    for (ExecutableElement containedMethod :
      ...
      if (MoreElements.isAnnotationPresent(containedMethod, AutoJson.Validate.class)) {
        if (containedMethod.getModifiers().contains(Modifier.STATIC)) {
          errorReporter.reportError(
              "@AutoJson.Validate cannot apply to a static method", containedMethod);
        } else if (!containedMethod.getParameters().isEmpty()) {
          errorReporter.reportError(
              "@AutoJson.Validate method must not have parameters", containedMethod);
        } else if (containedMethod.getReturnType().getKind() != TypeKind.VOID) {
          errorReporter.reportError(
              "Return type of @AutoJson.Validate method must be void", containedMethod);
        } else if (validateMethod.isPresent()) {
          errorReporter.reportError(
              "There can only be one @AutoJson.Validate method", containedMethod);
        } else {
  ...
  /**
   * Representation of an {@code AutoJson.Builder} class or interface.
   */
  class Builder {
  ...
  /**
   * Returns a representation of the given {@code @AutoJson.Builder} class or interface. If the
  ...
  private Optional<Builder> builderFrom(
    ...
    // We require the builder to have the same type parameters as the @AutoJson class, meaning the
    ...
    // builderTypeElement. A build method has no arguments and returns the @AutoJson class.
