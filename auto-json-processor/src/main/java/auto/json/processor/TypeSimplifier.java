...
final class TypeSimplifier {
   ...
   * the package containing the AutoJson class, provided that the result refers to the type
  ...
  // If we have @AutoJson abstract class Foo<T extends SomeClass> then this method will
  ...
  // If we have @AutoJson abstract class Foo<T extends Something> then the subclass will be
  // final class AutoJson_Foo<T extends Something> extends Foo<T>.
