 ...
 * the class with the {@code @AutoJson} annotation, and we do just enough parsing of that code to
 ...
 * {@code @AutoJson} constructor calls like {@code new AutoJson_Foo(...)} suddenly start being
...
class EclipseHack {
   ...
   * has no effect.
   */
  void reorderProperties(List<AutoJsonProcessor.Property> properties) {
    ...
    while (index < properties.size()) {
      ...
      List<AutoJsonProcessor.Property> subList = properties.subList(index, nextIndex);
