...
public class VelocityClassLoaderTest extends TestCase {
   ...
   * an {@code @AutoFacebook} class. If Velocity loads its managers using the context class loader,
  ...
  public static class RunInClassLoader implements Runnable {
    ...
    public void run() {
      String source = Joiner.on('\n').join(ImmutableList.of(
          "package foo.bar;",
          "import " + AutoFacebook.class.getName() + ";",
          "@AutoFacebook abstract class Test {",
          "  abstract int baz();",
          "  static Test create(int baz) {",
          "    return new AutoFacebook_Test(baz);",
          "  }",
          "}"));
