 ...
 * the class with the {@code @AutoFacebook} annotation, and we do just enough parsing of that code to
 ...
 * {@code @AutoFacebook} constructor calls like {@code new AutoFacebook_Foo(...)} suddenly start being
...
class EclipseHack {
   ...
   * has no effect.
   */
  void reorderProperties(List<AutoFacebookProcessor.Property> properties) {
    ...
    while (index < properties.size()) {
      ...
      List<AutoFacebookProcessor.Property> subList = properties.subList(index, nextIndex);
      reorderProperties(owner, subList);
      index = nextIndex;
    }
  }

  private void reorderProperties(TypeElement type, List<AutoFacebookProcessor.Property> properties) {
    ...
    for (AutoFacebookProcessor.Property property : properties) {
    ...
    if (allFound) {
      ...
      Comparator<AutoFacebookProcessor.Property> comparator = new Comparator<AutoFacebookProcessor.Property>() {
        @Override
        public int compare(AutoFacebookProcessor.Property a, AutoFacebookProcessor.Property b) {
