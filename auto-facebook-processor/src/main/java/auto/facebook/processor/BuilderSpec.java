...
/**
 * Support for AutoFacebook builders.
 *
...
class BuilderSpec {
  ...
  /**
   * Determines if the {@code @AutoFacebook} class for this instance has a correct nested
   * {@code @AutoFacebook.Builder} class or interface and return a representation of it in an
   * {@code Optional} if so.
   */
  Optional<Builder> getBuilder() {
    ...
    for (TypeElement containedClass : ElementFilter.typesIn(autoValueClass.getEnclosedElements())) {
      if (MoreElements.isAnnotationPresent(containedClass, AutoFacebook.Builder.class)) {
        if (!CLASS_OR_INTERFACE.contains(containedClass.getKind())) {
          errorReporter.reportError(
              "@AutoFacebook.Builder can only apply to a class or an interface", containedClass);
    ...
    for (ExecutableElement containedMethod :
      ...
      if (MoreElements.isAnnotationPresent(containedMethod, AutoFacebook.Validate.class)) {
        if (containedMethod.getModifiers().contains(Modifier.STATIC)) {
          errorReporter.reportError(
              "@AutoFacebook.Validate cannot apply to a static method", containedMethod);
        } else if (!containedMethod.getParameters().isEmpty()) {
          errorReporter.reportError(
              "@AutoFacebook.Validate method must not have parameters", containedMethod);
        } else if (containedMethod.getReturnType().getKind() != TypeKind.VOID) {
          errorReporter.reportError(
              "Return type of @AutoFacebook.Validate method must be void", containedMethod);
        } else if (validateMethod.isPresent()) {
          errorReporter.reportError(
              "There can only be one @AutoFacebook.Validate method", containedMethod);
        } else {
  ...
  /**
   * Representation of an {@code AutoFacebook.Builder} class or interface.
   */
  class Builder {
     ...
     * parameters must be the same as those of the @AutoFacebook class. Here's a correct example:
     * <pre>
     * {@code @AutoFacebook abstract class Foo<A extends Number, B> {
  ...
  /**
   * Returns a representation of the given {@code @AutoFacebook.Builder} class or interface. If the
  ...
  private Optional<Builder> builderFrom(
    ...
    // We require the builder to have the same type parameters as the @AutoFacebook class, meaning the
    ...
    // builderTypeElement. A build method has no arguments and returns the @AutoFacebook class.
