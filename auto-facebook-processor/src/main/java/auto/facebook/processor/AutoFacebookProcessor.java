 ...
 * @see auto.facebook.AutoFacebook
 */
@AutoService(Processor.class)
public class AutoFacebookProcessor extends AbstractProcessor {
  public AutoFacebookProcessor() {
  }

  @Override
  public Set<String> getSupportedAnnotationTypes() {
    return ImmutableSet.of(AutoFacebook.class.getName());
  }
  ...
  /**
   * Qualified names of {@code @AutoFacebook} classes that we attempted to process but had to abandon
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    ...
    if (roundEnv.processingOver()) {
      ...
      // any new instances of @AutoFacebook; and we can't have any new types that are the reason a type
      // was in deferredTypes.
      for (TypeElement type : deferredTypes) {
        errorReporter.reportError("Did not generate @AutoFacebook class for " + type.getQualifiedName()
            + " because it references undefined types", type);
      }
      return false;
    }
    Collection<? extends Element> annotatedElements =
        roundEnv.getElementsAnnotatedWith(AutoFacebook.class);
    ...
    for (TypeElement type : types) {
      try {
        ...
        // again and adding it back to the list). We save the name of the @AutoFacebook type rather
        ...
        errorReporter.reportError("@AutoFacebook processor threw an exception: " + trace, type);
      }
    }
  ...
  private String generatedSubclassName(TypeElement type) {
    return generatedClassName(type, "AutoFacebook_");
  }

  /**
   * A property of an {@code @AutoFacebook} class, defined by one of its abstract methods.
  ...
  public static class Property {
    ...
    /**
     * Returns the name of the getter method for this property as defined by the {@code @AutoFacebook}
  ...
  private void processType(TypeElement type) {
    AutoFacebook autoValue = type.getAnnotation(AutoFacebook.class);
    if (autoValue == null) {
      // This shouldn't happen unless the compilation environment is buggy,
      // but it has happened in the past and can crash the compiler.
      errorReporter.abortWithError("annotation processor for @AutoFacebook was invoked with a type"
          + " that does not have that annotation; this is probably a compiler bug", type);
    }
    if (type.getKind() != ElementKind.CLASS) {
      errorReporter.abortWithError(
          "@" + AutoFacebook.class.getName() + " only applies to classes", type);
    }
    if (ancestorIsAutoFacebook(type)) {
      errorReporter.abortWithError("One @AutoFacebook class may not extend another", type);
    }
    if (implementsAnnotation(type)) {
      errorReporter.abortWithError("@AutoFacebook may not be used to implement an annotation"
          + " interface; try using @AutoAnnotation instead", type);
    }
    AutoFacebookTemplateVars vars = new AutoFacebookTemplateVars();
  ...
  private void defineVarsForType(TypeElement type, AutoFacebookTemplateVars vars) {
    Types typeUtils = processingEnv.getTypeUtils();
    ...
    // Check for @AutoFacebook.Builder and add appropriate variables if it is present.
    if (builder.isPresent()) {
    ...
    for (Map.Entry<ExecutableElement, String> entry : methodToIdentifier.entrySet()) {
      if (!seen.add(entry.getValue())) {
        errorReporter.reportError(
            "More than one @AutoFacebook property called " + entry.getValue(), entry.getKey());
      }
    }
  }
  ...
  private static void determineObjectMethodsToGenerate(
      List<ExecutableElement> methods, AutoFacebookTemplateVars vars) {
      ...
      if (method.getModifiers().contains(Modifier.ABSTRACT)
        ...
        if (method.getParameters().isEmpty() && method.getReturnType().getKind() != TypeKind.VOID) {
          if (isReferenceArrayType(method.getReturnType())) {
            errorReporter.reportError("An @AutoFacebook class cannot define an array-valued property"
          ...
          // that overrides it. This shows up in AutoFacebookTest.LukesBase for example.
          errorReporter.reportWarning("@AutoFacebook classes cannot have abstract methods other than"
  ...
  private boolean ancestorIsAutoFacebook(TypeElement type) {
    while (true) {
      ...
      if (parentElement.getAnnotation(AutoFacebook.class) != null) {
        return true;
      }
      type = parentElement;
    }
  ...
  // The @AutoFacebook type, with a ? for every type.
  // If we have @AutoFacebook abstract class Foo<T extends Something> then this method will return
  // just <?>.
